...
| 日影規制 | 56条の2・別表第4 | 冬至日の真太陽時 8〜16時 (北海道 9〜15時・緩和値) を数値積分した等時間日影シミュレーション。対象建物判定 (軒高 7m 超/3階以上・高さ 10m 超)、測定面 (GL+1.5m/4m/6.5m)、5m・10m ライン、道路緩和 (施行令135条の12) |
...
| PD: 増築部の高さ | 同 A.1(f)/(h)・A.2(c) | 単層 4m、2 層以上は奥行 3m 以内かつ後方境界から 7m 以上。指定地では 2 層以上の増築は一律 PD 不可 |
| PD: 境界 2m 以内の軒高 | 同 A.1(i) | 敷地境界 (側面・後方) 2m 以内は軒高 3m 以下 |
| PD: 敷地被覆率 | 同 A.1(b) | curtilage (オリジナル住宅除く) の 50% 以下 |
...
- **プリセットシナリオ** — 低層戸建・中高層マンション・商業オフィス・狭小地3階建の代表的な検討条件を一括読込
...
- [Vitest](https://vitest.dev/) — 規制エンジンのユニットテスト 80 件超 (手計算による法規値と照合)
...
- 敷地は矩形・平坦で、南側の一辺のみが道路に接する (角地フラグは建蔽率 +10% のみに作用し、2 方向接道時の道路斜線 (令132条) や容積率の最大幅員採用はモデル外)
